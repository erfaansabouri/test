...
class CustomerController extends Controller {
    ...
    public function index ( Request $request ) {
        ...
        $customers = Customer::interactWithStore($store_manager->store_id)
                             ->with([
                                        'storeProfiles' => function ( $query ) use ( $store_manager ) {
    ...
    public function store ( Request $request ) {
        ...
        return redirect()
            ->route('store-manager.customers.edit' , $customer->id)
            ->with([
    ...
    private function updateOrCreate ( Customer $record , Request $request ) {
        ...
        if ( !$record->created_by_store_id ) {
            $record->created_by_store_id = Auth::guard('store-manager')
                                               ->user()->store_id;
        }
    ...
    public function levelsChart ( Request $request ) {
        ...
        foreach ( $levels as $level ) {
            $level->customers_count = CustomerStore::where('store_id' , $store_manager->store_id)
                                                   ->whereBetween('stars' , [
                                                       $level->min_stars_count ,
                                                       $level->max_stars_count ,
                                                   ])
                                                   ->count();
        }
